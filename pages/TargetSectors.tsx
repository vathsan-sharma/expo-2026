      ...
      <section className="py-32 max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          {SECTORS.map((sector, idx) => (
            <div 
              key={sector.id} 
              className="reveal glass-card rounded-sm p-12 md:p-20 border border-white/10 flex flex-col md:flex-row gap-12 group hover:border-brand-coral transition-all duration-700"
            ...
            >
              ...
              <div className="flex-grow">
                <div className="flex items-center gap-4 mb-8">
                  <h3 className="text-3xl md:text-4xl font-black tracking-tighter text-white leading-none italic">{sector.title}</h3>
                </div>
                <p className="text-white/60 text-xl font-medium mb-12 leading-relaxed italic border-l-4 border-brand-coral/20 pl-8">{sector.description}</p>
                <div className="space-y-6">
                  <h4 className="text-[11px] font-bold text-brand-coral tracking-tight mb-4 italic">Core Sub-Sectors</h4>
                  <ul className="grid grid-cols-1 gap-4">
                    {sector.subsectors?.map((sub, sIdx) => (
                      <li key={sIdx} className="flex items-center gap-4 text-xs font-bold tracking-tight text-white/40 group-hover:text-white group-hover:translate-x-4 transition-all duration-500 italic">
